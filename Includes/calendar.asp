...
If Request.Form<>"" then
	...
	AmountDue = Request.Form("tbxAmountDue")
...
%>[<%
	SQL = "SELECT EventId, EventTitle, EventLong, EventColor, EventPublished, EventExpires, EventAllDay, AmountDue FROM Events WHERE EventGroup = 1 OR Username = '" & Session("MM_Username") & "' ORDER BY EventId ASC"
	RS.Open SQL, Conn
...
ElseIf EventId<>"" then
	SQL = "SELECT EventId, EventGroup, EventTitle, EventLong, EventColor, EventPublished, EventExpires, EventAllDay, Reminder, Blast, isNull(RecurringPeriodId,0) as RecurringPeriodId, AmountDue FROM Events WHERE EventId=" & EventId & " ORDER BY EventId ASC"
	RS.Open SQL, Conn
	If not RS.EOF then Events = RS.GetRows
	RS.Close
	...
	If IsArray(Events) then
		For i=0 to UBound(Events,2)
            ...
            Reminder = Events(8,i)
            Blast = Events(9,i)
            RecurringPeriodId = Events(10,i)
            AmountDue = Events(11,i)
		Next
	End if
	Action = 2
...
<% If EventId<>"" and Request.QueryString("edit")="" then %>
  ...
  <h2><%=EventTitle%><% If EventGroup=1 then %><br />
...
<% End if %>
  <p style="border-top:1px solid #dfe0e4; padding-top:15px;">
  <%=AutoUrl(Replace(EventLong, Chr(10), "<br />" & VbCrLf))%></p>
<% If IsNumeric(AmountDue) Then
	strAmountDue = FormatCurrency(AmountDue)
   Else
	strAmountDue = "N/A"
   End If
%>
  <p>Amount Due: <%=strAmountDue%></p>
...
<% Else %>
  ...
  <form id="form" onsubmit="return Validation('includes/calendar.asp?id=<%=EventId%>&amp;action=<%=Action%>', $(this));">
    ...
    <p>Amount Due</p>
    <input name="tbxAmountDue" type="text" class="req" value="<%=AmountDue%>" size="8">
    ...
    <p>Send 15 day blast reminders? <input type="checkbox" class="checkbox" name="Blast" value="true" <% if Blast = true then response.write "checked" end if %> /></p>
    <p>Color</p>
...
Sub Add
	...
	Conn.Execute "INSERT INTO Events (EventGroup, EventTitle, EventLong, EventColor, EventViews, EventPublished, EventExpires, EventAllDay, EventDate, AmountDue, Reminder, Blast, RecurringPeriodId, Username) VALUES (" & _
        EventGroup & ", '" & EventTitle & "', '" & EventLong & "', '" & EventColor & "', 0, '" & EventPublished & "', '" & EventExpires & "', " & BoolToBit(EventAllDay) & _
        ", getdate(), " & AmountDue & ", " & BoolToBit(Reminder) & ", " &  BoolToBit(Blast) & ", " & iif(isnull(RecurringPeriodId), "null", RecurringPeriodId) & ", '" &  Session("MM_Username") & "' )"
...
Sub Edit(EventId)
    ...
    SQL = "UPDATE Events SET EventGroup=" & EventGroup & ", EventTitle='" & EventTitle & "', EventLong='" & EventLong & "', EventColor='" & EventColor & _
        ...
        ", EventUpdate=getdate(), AmountDue=" & AmountDue & ", Reminder=" & BoolToBit(Reminder) & ", Blast=" & BoolToBit(Blast) & ", RecurringPeriodId=" & iif(isnull(RecurringPeriodId), "null", RecurringPeriodId) & _
