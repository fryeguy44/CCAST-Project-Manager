...
Class ASPForm
	...
	Private Function IsInSizeLimit(TotalBytes)
		IsInSizeLimit = 1'(m_SizeLimit = 0 or m_SizeLimit > TotalBytes) and (MaxLicensedLimit > TotalBytes)
	End Function
...
Class cFormFields
	...
	Public Sub Save(Path, DestFileName)
		Dim Filename:Filename = DestFileName
		Dim Item
...
Function GetFileName(FullPath)
  ...
  'GetFileName = year(now()) & month(now()) & day(now()) & "_" & hour(now()) & minute(now()) & second(now())
End Function
